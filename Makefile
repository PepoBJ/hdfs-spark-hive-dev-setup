...
spark_home := $(addsuffix tools/spark-2.3.4-bin-without-hadoop, $(current_dir))
...
download_hadoop:
	...
	cd ${current_dir}tools; wget https://archive.apache.org/dist/hadoop/core/hadoop-2.7.2/hadoop-2.7.2.tar.gz && tar -xvf hadoop-2.7.2.tar.gz && rm -rf hadoop-2.7.2.tar.gz

download_spark:
	mkdir -p ${current_dir}tools
	cd ${current_dir}tools; wget https://archive.apache.org/dist/spark/spark-2.3.4/spark-2.3.4-bin-without-hadoop.tgz && tar -xvf spark-2.3.4-bin-without-hadoop.tgz && rm -rf spark-2.3.4-bin-without-hadoop.tgz

download_hive:
	mkdir -p ${current_dir}tools
	cd ${current_dir}tools; wget https://archive.apache.org/dist/hive/hive-2.1.0/apache-hive-2.1.0-bin.tar.gz && tar -xvf apache-hive-2.1.0-bin.tar.gz && rm -rf apache-hive-2.1.0-bin.tar.gz
